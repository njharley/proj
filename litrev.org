...
- Figure XXXX shows
- These plots form a useful reference guide when selecting parameter
  in values for the visualisation techniques described in
  [[Visualisation]].
...
- chord comparison plots
  - for choosing a comparison set
  - maximum discrimination
- class scape comparison
- measures tend to agree
  - high degree of correlation
- time scale
- mir chord recognition
- repertiore
  - billboard
  - beatles
*** Repertoire
...
- As mentioned in [[PC-set Theory for Analysis]], segmentation is a very
  important stage of the analysis. The segmentation policy used
  strongly influeces the SC content that can be observed, with larger
  segments tending to contain higher cardinality sets.
- Using a sliding window to segment the music requires careful
  consideration of the specific window length and hop size depending
  not only on the type and cardinality of sets that the analyst
  desires to observe but also on the way in which the data will then
  be visualised. 
- In this chapter we outline two segmentation policies:
  - A sliding window with fixed window length and hopsize.
  - The systematic segmentation policy of \citet{Martorell2013}.
** Systematic Segmentation
- \citet{Martorell2013} specifies a fully systematic segmentation
  policy which exhaustivly records every change in the PC material of
  a piece. The segments are of variable length but together capture
  the entire SC contents of the music. With each segment indexed in
  time by its center, the data structure, known as a cass-scape, is a
  sparse 3-dimensional binary matrix with axes representing time,
  time-scale (segment length) and set class.
- Two, more compact, representations of this data are the class-matrix
  and class-vector.
- Class-matrix: The class-matrix is a 2d projection the class-scape
  obtained by removing the time-scale information. This, in effect,
  acts to expand each point to the actual duration of the segment it
  represents.
- Class-vector: The class-vector is a further reduction of data
  showing the relative active duration of each class in the
  class-matrix and is expressed as a percentage of the total duration
  of the piece.
- EXAMPLE:
** Sliding Window
- Segmentation using a sliding window with fixed window and hop size
  is used to obtain a SC time series of one specific time-scale. This
  time series is then used for the visualisation techniques outlined
  in the next chapter.
- The purpose of the sliding window is to allow the analyst to focus
  on particular sets or tune in to one particular cardinality. This
  tuning process is performed through selection of an appropriate
  window and hop size.
- The selection of window and hop size can be informed by exploration
  of the data obtained from the systematic segmentation.
- SC v seg length
- Cardinality v seg length
* Visualisation
- proposes 3 methods for visualising sc descriptions of segmented
  pieces.
** Distance Plot
- A simple visualisation technique involves segmenting the piece with
  a fixed sliding window and then calculating the distance of each
  segment to a comparison set. The resulting distance time series can
  visualised with a simple line plot.
- Example: ???
- how was the segmentation done and why?
- how was the comparison set chosen?
- Compare plots with the different measures.
- recurring patterns?
** Auto-correlation
- The distance plots described in the previous section can give a
  basic view of the tonal progression of a piece. In many cases they
  contain recurring patterns where passages with similar SC content
  produce similar curves. A further visualisation technique is
  ...
  will have an autocorrelation plot with a slight peak corresponding
  to beginning of the repeat of section A. 

- Example:

- Structural segmentation
** Self-similarity matrix
- As an alternative to using a specific comparison set, the distance
  of a segment to other segments in the piece could be observed. A
  systematic way of doing this is computing the self-similarity matrix
  which provides a simples means for descovery of repetitions in a
  time series. The patterns present in a visualisation of a
  self-similarity matrix can indicate similar passages and structural
  elements of a piece.
- Example: ????
** Conclusions
- 
* Analysis Tool
- Due to the complex relationship between the selection of the various
